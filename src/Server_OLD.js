...
// Nova rota cubix-ai nos mesmos moldes da anterior
app.post("/api/cubix-ai", async (req, res) => {
  // Extrai dados do body
  const { message, sessionId, userName, userId, userEmail } = parseBody(req.body);
  const finalSessionId = sessionId || uuidv4();
  ...
  try {
    const flowResp = await fetch(
      "https://n8n.altavistainvest.com.br/webhook/cubix-ai",
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        timeout: 180000, // 3 minutos
    ...
    // Logar no console
    console.log("[DEBUG] JSON enviado ao n8n (cubix-ai):", JSON.stringify({
      userMessage: message,
      sessionId: finalSessionId,
      userName,
      userId,
    ...
    console.error("[DEBUG] Erro ao chamar Flow (cubix-ai):", err);
    return res.status(500).json({ error: "Falha ao chamar Flow" });
  }
});
